...
const config = { repoDirectory: "", allowPush: true, enableGitHub: false, maxOutputChars: 20000 };
...
const names = async () =>
  (await toolsProvider(fakeController({ config: { ...config, repoDirectory: repo }, workingDirectory: work }))).map(t => t.name);
