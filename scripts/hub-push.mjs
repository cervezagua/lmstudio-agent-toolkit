// Publishes plugins to LM Studio Hub from clean staging copies. Run `lms login` first.
//
...
const ok = withStagedPlugins(plugins, (name, stage) => {
  ...
  if (manifest.owner === "local") {
    console.error(`\n${name}: manifest owner is "local". Pass --owner <your-hub-account> to publish.`);
    return false;
  }
  const target = `${manifest.owner}/${manifest.name}${isPrivate ? " (private)" : ""}`;
  if (dryRun) {
    console.log(`\n=== Would push ${target} from a clean copy of plugins/${name}`);
    return true;
  }
  console.log(`\n=== Pushing ${target}`);
